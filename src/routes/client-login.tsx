...
export const Route = createFileRoute("/client-login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in to view your Brix projects, assets, and guidelines.",
      },
...
function ClientLoginPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (active && data.session) navigate({ to: "/client/dashboard" });
    });
    return () => {
      active = false;
    };
  ...
  const field =
    "w-full rounded-md border border-paper/15 bg-paper/[0.06] px-4 py-3 text-paper placeholder:text-paper/35 focus-visible:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/50";
  ...
  return (
    <main className="portal-auth-theme min-h-svh bg-obsidian text-paper lg:grid lg:grid-cols-2">
      <section className="flex min-h-svh flex-col px-5 py-6 sm:px-10 lg:px-16">
        <img src={wordmarkDark} alt="Brix Design Studio" className="h-4 w-auto self-start" />
        <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center py-12">
          ...
          <form onSubmit={signIn} className="space-y-4">
            <div>
              <label htmlFor="client-email" className="label-caps mb-2 block text-paper/55">
                Email
              </label>
              <input
                id="client-email"
            ...
            <div>
              <label htmlFor="client-password" className="label-caps mb-2 block text-paper/55">
                Password
              </label>
              <input
                id="client-password"
          ...
          <p className="mt-6 text-sm text-paper/50">
            New here? Use the secure link in your Brix invitation email.
          </p>
          <Link to="/portal" className="mt-4 text-sm text-paper/60 underline underline-offset-4">
            Studio team sign in
          </Link>
        </div>
      </section>
