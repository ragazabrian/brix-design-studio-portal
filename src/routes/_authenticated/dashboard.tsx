...
function Integrations() {
  ...
  async function toggle(provider: string) {
    ...
    if (existing) {
      const { error } = await supabase
        ...
        .eq("id", existing.id);
      if (error) {
        toast.error("That connection was not removed.");
        return;
      }
    } else {
  ...
  return (
    <section aria-labelledby="integrations-heading" className="mt-10">
      <h2 id="integrations-heading" className="label-caps text-muted-foreground">
        Connections
      </h2>
      <ul className="mt-4 space-y-2">
        {providers.map((provider) => {
          ...
          return (
            ...
            >
              <span className="min-w-0">
                <span className="block truncate text-[15px]">{provider.label}</span>
