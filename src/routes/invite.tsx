...
function InvitePage() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      token ? getInvite({ data: { token } }) : Promise.resolve({ available: false as const, reason: "not_found" as const }),
  ...
  async function useGoogle() {
    ...
    const result = await lovable.auth.signInWithOAuth("google", {
      redirect_uri: `${window.location.origin}/invite?token=${encodeURIComponent(token)}`,
    });
    if (result.error) toast.error("Google sign in did not complete.");
  }

  const field = "w-full rounded-md border border-paper/15 bg-paper/[0.06] px-4 py-3 text-paper placeholder:text-paper/40 focus-visible:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/50";

  return (
    <main className="portal-auth-theme flex min-h-svh items-center justify-center bg-obsidian px-5 py-10 text-paper">
      <section className="w-full max-w-lg border border-paper/15 bg-paper/[0.04] p-6 sm:p-10" aria-live="polite">
        <img src={wordmarkDark.url} alt="Brix Design Studio" className="h-4 w-auto" />
        {loading ? <p className="mt-12 text-paper/60">Checking your invitation.</p> : !invite?.available ? (
          <div className="mt-12">
            <h1 className="text-3xl font-semibold">This invitation is unavailable</h1>
            <p className="mt-4 text-paper/60">It may have expired, been cancelled, or already been accepted. Ask your Brix contact for a new invitation.</p>
            <Button asChild className="mt-8"><a href="/client-login">Go to client sign in</a></Button>
          </div>
        ) : (
          <div className="mt-10">
            <HugeiconsIcon icon={Mail01Icon} size={28} strokeWidth={1.5} aria-hidden className="text-brand" />
            <p className="label-caps mt-6 text-paper/50">Portal invitation</p>
            <h1 className="mt-3 text-3xl font-semibold">Join Brix as {invite.role === "client" ? "a client" : invite.role === "designer" ? "a designer" : "an admin"}</h1>
            <p className="mt-4 text-paper/65">This invitation is for <strong className="text-paper">{invite.email}</strong>.</p>
            {invite.projects.length > 0 ? <p className="mt-2 text-paper/65">Projects: {invite.projects.join(", ")}</p> : null}
            {invite.note ? <p className="mt-5 border-l-2 border-brand pl-4 text-sm text-paper/70">{invite.note}</p> : null}
            ...
            {sessionEmail ? (
              ...
              <div className="mt-8">
                <Button type="button" onClick={useGoogle} variant="outline" className="h-11 w-full border-paper/20 bg-paper text-obsidian hover:bg-paper/90">Continue with Google</Button>
                <div className="my-5 flex items-center gap-3 text-xs text-paper/40"><span className="h-px flex-1 bg-paper/15" />or sign in<span className="h-px flex-1 bg-paper/15" /></div>
                <form onSubmit={signIn} className="space-y-4">
                  <div><label htmlFor="invite_email" className="label-caps mb-2 block text-paper/55">Email</label><input id="invite_email" name="email" type="email" readOnly value={invite.email} className={field} /></div>
                  <div><label htmlFor="invite_password" className="label-caps mb-2 block text-paper/55">Password</label><input id="invite_password" name="password" type="password" required className={field} /></div>
                  <Button type="submit" disabled={busy} className="h-11 w-full bg-brand text-paper hover:bg-brand-deep">{busy ? "Signing in" : "Sign in and accept"}</Button>
                </form>
              </div>
            )}
          </div>
